...
import {
  // 섹션 아이콘(전부 적용)
  Users,            // 1. Competition
  FileText,         // 2. Basic Information
  Banknote,         // 3. Financing Method
  Home,             // 4. Home Sale Contingency
  Coins,            // 5. EMD
  Search,           // 6. Inspection
  ShieldCheck,      // 7. Appraisal & Fin. Cont.
  FileSignature,    // 8. Taxes/Title
  Handshake,        // 9. Commission
  TrendingUp,       // 10. Offer & Escalation
  Sparkles,         // Final Strength
  RefreshCcw,
  Download,
} from "lucide-react";
...
function OptionTile({
  ...
  const outerStyle: React.CSSProperties = {
    ...
    boxShadow: active ? "0 0 0 3px rgba(255,255,255,.06)" : "none",
  };
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
        ...
        >
          {active ? <span style={{ display: "block", width: 12, height: 12, borderRadius: 999, background: "#111" }} /> : null}
        </span>
...
export default function App() {
  ...
  return (
    <div className="mc-bg" style={{ minHeight: "100vh" }}>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "36px 20px 56px" }}>
        {/* Header (centered) */}
        <div style={{ textAlign: "center" }}>
            ...
            <CardContent className="p-5 md:p-6">
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 8 }}>
                  <Slider value={[downPct]} min={0} max={100} step={1} onValueChange={(v: number[]) => setDownPct(v[0])} className="w-full" />
          ...
          <Card className="mc-card">
            <CardContent className="p-5 md:p-6">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 12 }}>
                <Slider value={[emdPct]} min={0} max={20} step={1} onValueChange={(v: number[]) => setEmdPct(v[0])} className="w-full" />
            ...
            <CardContent className="p-6 md:p-8" style={{ textAlign: "center" }}>
              ...
              <div style={{ marginTop: 18 }}>
                ...
                <ul style={{ listStyle: "disc", paddingLeft: 18, margin: "0 auto", textAlign: "left", maxWidth: 680, lineHeight: 1.6 }}>
                  {getRecommendations(getRecsState()).map((r, i) => (<li key={i} style={{ fontSize: 14 }}>{r}</li>))}
                </ul>
              </div>
...
Recommendations:
${getRecommendations(state).map((x,i)=>`${i+1}. ${x}`).join("\n")}
`;
